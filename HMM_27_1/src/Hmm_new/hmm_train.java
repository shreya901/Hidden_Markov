...
public class hmm_train {
	...
	public static model_log  train(int [] obs_seq,int itr,int hidden_state)
	{
		int max_itr=itr;
		int N=hidden_state;
		double old_log_prob=Integer.MIN_VALUE;
		hmm_train my_hmm=new hmm_train();
		model m =my_hmm.get_initial_model(N,obs_seq.length);
		for (int i =0;i<max_itr;i++)
		{
			...
			model m_new=my_hmm.re_estimation(m, obs_seq, beta, ac, gg);
	...
	private model re_estimation(model m,int[] O,double beta[][],alpha_c ac,gamma_gammasum gg)
	{
		int N=m.a.length;
		int T=m.T;
		int K=27;
		double a[][]=new double[N][N];
		double b[][]=new double[N][27];
	...
	private alpha_c alpha_pass(model m,int[] O)
		{
			int N=m.a.length;
			int T=m.T;
			double alpha_bar[][]=new double [T][N];
			double c[]=new double[T];
			for (int i =0;i<N;i++)
			{
				alpha_bar[0][i]=m.pi[i]*m.b[i][O[0]];
				c[0]+=alpha_bar[0][i];
			...
			for (int i =0;i<N;i++)
			{
				alpha_bar[0][i]=c[0]*alpha_bar[0][i];
			}
			for (int t=1;t<T;t++)
			{
				for (int i =0;i<N;i++)
				{
					...
					alpha_bar[t][i]=alpha_bar[t][i]*m.b[i][O[t]];
					c[t]+=alpha_bar[t][i];
				...
				for (int i =0;i<N;i++)
					...
					alpha_bar[t][i]=c[t]*alpha_bar[t][i];
	...
	class model
	{
		...
		double pi[];
		int T;
		model(double a[][],double b[][],double pi[],int T)
		{
			this.a=a;
			this.b=b;
			this.pi=pi;
	...
	private  model get_initial_model(int N,int T)
	{
		double pi[]=new double[N];
		double a[][]= new double[N][N];
		double b[][]=new double[N][27];
		...
		for (int i =0;i<N;i++)
		{
			pi[i]=(double)Math.abs((ran.nextGaussian()*0.5+var));
			sum_pi+=pi[i];
		...
		for (int i =0;i<N;i++)
		{
			for (int j =0;j<N;j++)
			{
				a[i][j]=(double)Math.abs((ran.nextGaussian()*0.5+var));
				sum_a[i]+=a[i][j];
			}
		}
		...
		for (int i =0;i<N;i++)
		{
			for (int j =0;j<27;j++)
			{
				b[i][j]=(double)Math.abs((ran.nextGaussian()*0.037+var));
				sum_b[i]+=b[i][j];
			}
		}
