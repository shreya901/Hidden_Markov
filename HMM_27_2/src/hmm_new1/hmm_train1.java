...
public class hmm_train1 {
	
	
	

	public static model_log  train(int [] obs_seq,int itr,int hidden_state, float[][] A)
	{
		int max_itr=itr;
		int N=hidden_state;
		double old_log_prob=Integer.MIN_VALUE;
		hmm_train1 my_hmm=new hmm_train1();
		model m =my_hmm.get_initial_model(N,obs_seq.length,A);
	...
	private  model get_initial_model(int N,int T, float[][] A)
	{
		double pi[]=new double[N];
		float a[][]= A;
		double b[][]=new double[N][27];
